...
#   v1.4.9.alpha-002       - 版本号（用于 version 字段，如 "0.0.1"）
#   branch - 安装类型: "tag" 或 "branch"
#   feature/fix-linux-packaging-error  - 版本引用: tag 名称（如 "v0.0.1"）或 branch 名称（如 "main"）
class Workflow < Formula
  ...
  on_macos do
    ...
    # 构建脚本会根据 branch 的值来替换下面的占位符：
    # - 如果 branch == "tag"，则替换为: url "...", tag: "feature/fix-linux-packaging-error"
    # - 如果 branch == "branch"，则替换为: url "...", branch: "feature/fix-linux-packaging-error"
    #
    ...
    #   fi
    url "https://github.com/zevwings/workflow.rs.git", branch: "feature/fix-linux-packaging-error"
    # 示例（构建脚本会生成以下之一）:
